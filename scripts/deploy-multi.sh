...
# Mirrors the `quality` job in .github/workflows/ci.yml and the gate in
# deploy.sh — keep all three in sync.
run_quality_gate() {
...
build_app() {
    ...
    # Gate before build so no unverified artifact is ever produced. Called here
    # rather than from main() because build_app has two call sites.
    run_quality_gate
